...
func (u *AuthUsecase) Register(email, password string) (*entities.User, error) {
...
func (u *AuthUsecase) Login(email, password string) (string, error) {
	...
	token, err := sharedJWT.GenerateToken(user.ID, user.Email, user.Role, u.cfg.JWTSecret, 24*time.Hour)
	if err != nil {
		return "", err
	}
...
func (u *AuthUsecase) sendEmail(to, code string) error {
	...
	return smtp.SendMail(addr, auth, u.cfg.SMTPUser, []string{to}, []byte(msg))
}
